...
* [ThreadStudy](#threadstudy)
	* [1.线程状态？](#1.线程状态)
	* [2.线程池状态？](#2线程池状态)
	* [3.线程池中 submit() 和 execute() 有什么区别？](#3线程池中-submit-和-execute-有什么区别)
	* [4.ThreadLocal是什么？](#4threadlocal是什么)
	* [5.说一下synchronized底层实现原理？](#5说一下synchronized底层实现原理)
	* [6. sleep()和wait()的区别](#6-sleep和wait的区别)
	* [7.synchronized 和 Lock的区别](#7synchronized-和-lock的区别)
	* [8.List是线程不安全的，如何解决？](#8list是线程不安全的如何解决)
	* [9.HashSet是线程不安全的，如何解决？](#9hashset是线程不安全的如何解决)
	* [10.HashMap是线程不安全的，如何解决？](#10hashmap是线程不安全的如何解决)
	* [11.Callable和Runnable有什么区别？](#11callable和runnable有什么区别)
	* [12.如何确保若干个线程都执行完毕才执行接下来的逻辑？](#12如何确保若干个线程都执行完毕才执行接下来的逻辑)
	* [13.如何集齐7颗龙珠 才能 召唤神龙？](#13如何集齐7颗龙珠-才能-召唤神龙)
	* [14.有6个车，只有3个车位怎么办？](#14有6个车只有3个车位怎么办)
	* [15.读写锁有几把锁？2把吗？](#15读写锁有几把锁2把吗)
	* [16.创建线程池有多少种方式？](#16创建线程池有多少种方式)
	* [17.请按照给出数据，找出同时满足以下条件的用户](#17请按照给出数据找出同时满足以下条件的用户)
	* [18.从0加到20亿，可以用什么方法？](#18从0加到20亿可以用什么方法)
	* [19.异步调用](#19异步调用)	
	* [20. JMM](#20-jmm)
	* [21.volatile](#21volatile)
	* [21.CAS是什么](#21cas是什么)
	* [22. 公平锁、非公平锁](#22-公平锁-非公平锁)
	* [23. 可重入锁](#23-可重入锁)
	* [24.自旋锁](#24自旋锁)
	* [25.怎么排查死锁？](#25怎么排查死锁)
...
[详解](https://tech.meituan.com/2018/11/15/java-lock.html)
...
Mark Word：默认存储对象的HashCode，分代年龄和**锁标志位**信息。这些信息都是与对象自身定义无关的数据，所以Mark Word被设计成一个非固定的数据结构以便在极小的空间内存存储尽量多的数据。它会根据对象的状态复用自己的存储空间，也就是说在运行期间Mark Word里存储的数据会随着锁标志位的变化而变化。
...
如同在自旋锁中提到的“阻塞或唤醒一个Java线程**需要操作系统切换CPU状态来完成**，这种状态转换需要**耗费处理器时间**。如果同步代码块中的内容过于简单，状态转换消耗的时间有可能比用户代码执行的时间还要长”。
...
偏向锁是**指一段同步代码一直被一个线程所访问，那么该线程会自动获取锁，降低获取锁的代价**。
...
简而言之，偏向锁是1个线程持有锁。如果有第二个线程访问，这个锁就升级为轻量级锁。
...
简而言之，2个线程间，一个持有锁，一个在自旋。如果自旋次数过多 or 有第三个线程来访问 ---> 升级为重量级锁，

- 重量级锁

升级为重量级锁时，锁标志的状态值变为“10”，此时Mark Word中存储的是指向重量级锁的指针，此时等待锁的线程都会进入**阻塞**状态。
